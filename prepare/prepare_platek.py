...
def read_platek_dataset(path):
    ...
    normal_tweets = []
    with open(os.path.join(path, 'en-balanced', 'normal.tweets.tsv'), encoding="utf-8") as f:
        for line in f.readlines():
            index, string = line.strip().split('\t')[:2]
            if string != 'Not Available':
                tweet = Tweet(string, '0', index)
                normal_tweets.append(tweet)

    sarcastic_tweets = []
    with open(os.path.join(path, 'en-balanced', 'sarcastic.tweets.tsv'), encoding="utf-8") as f:
        for line in f.readlines():
            index, string = line.strip().split('\t')[:2]
            if string != 'Not Available':
