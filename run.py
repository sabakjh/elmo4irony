...
def main():
    ...
    # silently ignored if there are no GPUs
    ...
    if hp.model_hash:
        experiment_path = os.path.join(
            config.RESULTS_PATH, hp.model_hash + '*')
        ...
        # 读取batch size 和 hidden size的参数 并创建模型
    ...
    try:
        ...
        for epoch in tqdm(range(hp.epochs), desc='Epoch'):
            ...
            if hp.training_schedule == 'decay-nie':
                optim_updated, new_lr = trainer.optimizer.update_learning_rate_nie(
                    epoch)
                if optim_updated:
                    tqdm.write(f'Learning rate decayed to {new_lr}')
