...
class Lang(object):
    ...
    def _build_token_dict(self, sents):
        ...
        # 自动把特殊字符放到字典中
        token_dict = {key: value for key,
                      value in config.SPECIAL_TOKENS.items()}
        # 初始化token id
        curr_token_id = len(token_dict)
        token_freqs = defaultdict(int)
        # 统计所有token的频率，并分配一个id
        for sent in sents:
            for token in sent:
                token_freqs[token] += 1
    ...
    def sent2ids(self, sent, ignore_period=True, append_EOS=False):
        if not isinstance(sent, list):
            raise TypeError(
                f'Input shout be a list but got {type(sent)} instead.')
        ids = []
        for token in sent:
